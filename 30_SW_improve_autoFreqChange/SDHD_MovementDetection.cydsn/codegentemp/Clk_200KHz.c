...
void Clk_200KHz_Start(void) 
{
    ...
    Clk_200KHz_CLKEN |= Clk_200KHz_CLKEN_MASK;
	Clk_200KHz_CLKSTBY |= Clk_200KHz_CLKSTBY_MASK;
}
...
void Clk_200KHz_Stop(void) 
{
    ...
    Clk_200KHz_CLKEN &= (uint8)(~Clk_200KHz_CLKEN_MASK);
	Clk_200KHz_CLKSTBY &= (uint8)(~Clk_200KHz_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clk_200KHz_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clk_200KHz_CLKEN & Clk_200KHz_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clk_200KHz_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clk_200KHz_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clk_200KHz__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clk_200KHz_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clk_200KHz_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clk_200KHz_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clk_200KHz_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clk_200KHz_GetSourceRegister();
    uint16 oldDivider = Clk_200KHz_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clk_200KHz_CLKEN & Clk_200KHz_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clk_200KHz_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
{
    return CY_GET_REG16(Clk_200KHz_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clk_200KHz_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clk_200KHz_MOD_SRC |= modeBitMask & (uint8)Clk_200KHz_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clk_200KHz_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clk_200KHz_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clk_200KHz_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clk_200KHz_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clk_200KHz_MOD_SRC & (uint8)(Clk_200KHz_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clk_200KHz_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clk_200KHz_GetDividerRegister();
    uint8 oldSrc = Clk_200KHz_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clk_200KHz_MOD_SRC |= CYCLK_SSS;
        Clk_200KHz_MOD_SRC =
            (Clk_200KHz_MOD_SRC & (uint8)(~Clk_200KHz_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clk_200KHz_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clk_200KHz_MOD_SRC & Clk_200KHz_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clk_200KHz_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clk_200KHz_PHASE = clkPhase & Clk_200KHz_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clk_200KHz_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clk_200KHz_PHASE & Clk_200KHz_PHASE_MASK;
}
