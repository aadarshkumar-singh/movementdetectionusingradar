...
/** Maximum number of times the data is sent to testbench*/
#define MAXIMUM_COUNTS 10 

/** Maximum number of frequency variation possible*/
#define MAXIMUM_FREQ_VARIATION 10

/* Counter value for checking the no of times data is tx to matlab */
...
void movementDetector_StateMachine(uint16 * buffer , uint16 length)
{
    ...
    static  uint8_t autoFreqChang = 0;
    ...
    switch(state)
    {
        case STATE_IDLE :    
            ...
            if (getEvent(EV_BUTTON))
            {
                ...
                /* Change frequency by incrementing control register value to adjust Mux */
                MuxControlRegister_Write(autoFreqChang);
                autoFreqChang ++;
                
                /*Reset After 10 possible frequency variations*/
                if(autoFreqChang == MAXIMUM_FREQ_VARIATION)
                {
                    autoFreqChang = 0;
                }
