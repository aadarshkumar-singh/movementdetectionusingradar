...
/*******************************************************************************
* Function Name: Clk_350KHz_Start
********************************************************************************
*
* Summary:
...
void Clk_350KHz_Start(void) 
{
    ...
    Clk_350KHz_CLKEN |= Clk_350KHz_CLKEN_MASK;
	Clk_350KHz_CLKSTBY |= Clk_350KHz_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clk_350KHz_Stop
********************************************************************************
*
* Summary:
...
void Clk_350KHz_Stop(void) 
{
    ...
    Clk_350KHz_CLKEN &= (uint8)(~Clk_350KHz_CLKEN_MASK);
	Clk_350KHz_CLKSTBY &= (uint8)(~Clk_350KHz_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clk_350KHz_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clk_350KHz_CLKEN & Clk_350KHz_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clk_350KHz_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clk_350KHz_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clk_350KHz__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clk_350KHz_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clk_350KHz_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clk_350KHz_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clk_350KHz_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clk_350KHz_GetSourceRegister();
    uint16 oldDivider = Clk_350KHz_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clk_350KHz_CLKEN & Clk_350KHz_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clk_350KHz_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clk_350KHz_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clk_350KHz_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clk_350KHz_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clk_350KHz_MOD_SRC |= modeBitMask & (uint8)Clk_350KHz_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clk_350KHz_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clk_350KHz_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clk_350KHz_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clk_350KHz_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clk_350KHz_MOD_SRC & (uint8)(Clk_350KHz_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clk_350KHz_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clk_350KHz_GetDividerRegister();
    uint8 oldSrc = Clk_350KHz_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clk_350KHz_MOD_SRC |= CYCLK_SSS;
        Clk_350KHz_MOD_SRC =
            (Clk_350KHz_MOD_SRC & (uint8)(~Clk_350KHz_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clk_350KHz_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clk_350KHz_MOD_SRC & Clk_350KHz_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clk_350KHz_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clk_350KHz_PHASE = clkPhase & Clk_350KHz_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clk_350KHz_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clk_350KHz_PHASE & Clk_350KHz_PHASE_MASK;
}
